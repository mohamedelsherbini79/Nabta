...
export function ConsultationList({ consultations, now }: { consultations: ConsultationSummary[]; now: string }) {
  ...
  function doctorLabel(c: ConsultationSummary) {
    if (!c.doctor) return "—";
    return c.doctor.specialty ? `${c.doctor.name} — ${c.doctor.specialty}` : (c.doctor.name ?? "—");
  }

  return (
    <div className="flex flex-col gap-6">
      <section>
        <h2 className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">{t("consultations.upcoming.title")}</h2>
        {upcoming.length === 0 ? (
          <p className="text-sm text-zinc-400">{t("consultations.upcoming.empty")}</p>
        ) : (
          <ul className="flex flex-col gap-3">
            {upcoming.map((c) => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="text-sm font-medium text-zinc-900 dark:text-zinc-50">{doctorLabel(c)}</p>
      ...
      <section>
        <h2 className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">{t("consultations.past.title")}</h2>
        {past.length === 0 ? (
          <p className="text-sm text-zinc-400">{t("consultations.past.empty")}</p>
        ) : (
          <ul className="flex flex-col gap-3">
            {past.map((c) => (
              <li
