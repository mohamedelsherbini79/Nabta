...
import { getOrdersForProfile, toOrderSummary } from "@/lib/pharmacy";
...
export async function GET(req: Request) {
  ...
  const orders = await getOrdersForProfile(patientProfileId);
  return NextResponse.json({ orders: orders.map(toOrderSummary) });
}
