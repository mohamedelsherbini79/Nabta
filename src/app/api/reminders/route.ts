...
import { createReminder, getRemindersForProfile, toReminderSummary } from "@/lib/reminders";
...
export async function GET(req: Request) {
  ...
  const reminders = await getRemindersForProfile(patientProfileId);
  return NextResponse.json({ reminders: reminders.map(toReminderSummary) });
}
...
export async function POST(req: Request) {
  ...
  const parsed = reminderSchema.safeParse(body?.input);
  ...
  const reminder = await createReminder(patientProfileId, parsed.data);
  await logAudit({ actorUserId: user.id, action: "REMINDER_CREATE", entityType: "Reminder", entityId: reminder.id });

  return NextResponse.json({ reminder: toReminderSummary(reminder) }, { status: 201 });
}
