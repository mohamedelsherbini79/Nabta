...
import { createVaccinationRecord, getVaccinationsForProfile, toVaccinationRecordSummary } from "@/lib/vaccinations";
...
export async function GET(req: Request) {
  ...
  const records = await getVaccinationsForProfile(patientProfileId);
  return NextResponse.json({ records: records.map(toVaccinationRecordSummary) });
}
...
export async function POST(req: Request) {
  ...
  const parsed = vaccinationRecordSchema.safeParse(body?.input);
  ...
  const record = await createVaccinationRecord(patientProfileId, parsed.data);
  await logAudit({ actorUserId: user.id, action: "VACCINATION_CREATE", entityType: "VaccinationRecord", entityId: record.id });

  return NextResponse.json({ record: toVaccinationRecordSummary(record) }, { status: 201 });
}
