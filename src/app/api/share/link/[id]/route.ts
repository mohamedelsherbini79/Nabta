...
import { revokeDoctorShareLink } from "@/lib/doctorShare";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!link || link.purpose !== "DOCTOR_SHARE") {
  ...
  await revokeDoctorShareLink(id);
  await logAudit({ actorUserId: user.id, action: "DOCTOR_SHARE_LINK_REVOKE", entityType: "QrShareLink", entityId: id });
