...
export async function GET(req: Request) {
  ...
  const balance = await getBalance(patientProfileId);
  const ledger = await getLedgerForProfile(patientProfileId);

  return NextResponse.json({ balance, ledger: ledger.map(toLoyaltyLedgerEntrySummary) });
}
