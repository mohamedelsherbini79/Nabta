...
import { getGeneticProfile, toGeneticProfileSummary, upsertGeneticProfile } from "@/lib/genetics";
import { geneticProfileSchema } from "@/lib/validation";
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ profile: profile ? toGeneticProfileSummary(profile) : null });
}
...
export async function PUT(req: Request) {
  ...
  const parsed = geneticProfileSchema.safeParse(body?.input);
  ...
  const profile = await upsertGeneticProfile(patientProfileId, parsed.data);
  await logAudit({ actorUserId: user.id, action: "GENETIC_PROFILE_UPSERT", entityType: "GeneticProfile", entityId: profile.id });

  return NextResponse.json({ profile: toGeneticProfileSummary(profile) });
}
